...
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_update_health_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_update_health_t* ms = SGX_CAST(ms_ecall_update_health_t*, pms);
	ms_ecall_update_health_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ecall_update_health_t), ms, sizeof(ms_ecall_update_health_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	const char* _tmp_sourceType = __in_ms.ms_sourceType;
	size_t _len_sourceType = __in_ms.ms_sourceType_len ;
	char* _in_sourceType = NULL;
	int* _tmp_newHealth = __in_ms.ms_newHealth;
	size_t _len_newHealth = sizeof(int);
	int* _in_newHealth = NULL;

	CHECK_UNIQUE_POINTER(_tmp_sourceType, _len_sourceType);
	CHECK_UNIQUE_POINTER(_tmp_newHealth, _len_newHealth);
	...
	if (_tmp_sourceType != NULL && _len_sourceType != 0) {
		_in_sourceType = (char*)malloc(_len_sourceType);
		if (_in_sourceType == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_sourceType, _len_sourceType, _tmp_sourceType, _len_sourceType)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

		_in_sourceType[_len_sourceType - 1] = '\0';
		if (_len_sourceType != strlen(_in_sourceType) + 1)
		{
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
	if (_tmp_newHealth != NULL && _len_newHealth != 0) {
		if ( _len_newHealth % sizeof(*_tmp_newHealth) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_newHealth = (int*)malloc(_len_newHealth);
		if (_in_newHealth == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_newHealth, _len_newHealth, _tmp_newHealth, _len_newHealth)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	ecall_update_health(__in_ms.ms_playerId, __in_ms.ms_deltaHealth, (const char*)_in_sourceType, _in_newHealth);
	if (_in_newHealth) {
		if (memcpy_verw_s(_tmp_newHealth, _len_newHealth, _in_newHealth, _len_newHealth)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	if (_in_sourceType) free(_in_sourceType);
	if (_in_newHealth) free(_in_newHealth);
	return status;
}
