...
typedef struct ms_update_health_t {
	int ms_retval;
	int ms_current;
	int ms_damage;
	int ms_max;
...
static sgx_status_t SGX_CDECL sgx_update_health(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_update_health_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_update_health_t* ms = SGX_CAST(ms_update_health_t*, pms);
	ms_update_health_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_update_health_t), ms, sizeof(ms_update_health_t))) {
	...
	_in_retval = update_health(__in_ms.ms_current, __in_ms.ms_damage, __in_ms.ms_max);
...
SGX_EXTERNC const struct {
	...
	1,
	{
		{(void*)(uintptr_t)sgx_update_health, 0, 0},
	}
};
