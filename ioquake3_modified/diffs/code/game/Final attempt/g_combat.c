...
#ifdef ENABLE_SGX_PIPELINE
		if (targ->client) {
			DamageInput sgx_input = {
				.damage = damage,
				.armor = targ->client->ps.stats[STAT_ARMOR],
				.dflags = dflags
			};
			DamageOutput sgx_output;

			if (SGX_SimulateDamageValidation(&sgx_input, &sgx_output)) {
				take = sgx_output.final_damage;
				asave = sgx_output.final_armor;
				knockback = sgx_output.knockback;
