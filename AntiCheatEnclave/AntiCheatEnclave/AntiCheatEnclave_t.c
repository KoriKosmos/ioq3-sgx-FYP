...
static sgx_status_t SGX_CDECL sgx_ecall_encrypt_message(void* pms)
{
	...
	ms_ecall_encrypt_message_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ecall_encrypt_message_t), ms, sizeof(ms_ecall_encrypt_message_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_plaintext = __in_ms.ms_plaintext;
	size_t _tmp_msg_len = __in_ms.ms_msg_len;
	size_t _len_plaintext = _tmp_msg_len;
	uint8_t* _in_plaintext = NULL;
	const uint8_t* _tmp_iv = __in_ms.ms_iv;
	size_t _len_iv = 12;
	uint8_t* _in_iv = NULL;
	uint8_t* _tmp_ciphertext = __in_ms.ms_ciphertext;
	size_t _len_ciphertext = _tmp_msg_len;
	...
	_in_retval = ecall_encrypt_message((const uint8_t*)_in_plaintext, _tmp_msg_len, (const uint8_t*)_in_iv, _in_ciphertext, _in_mac);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
